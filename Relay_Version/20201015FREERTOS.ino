#include "20201015FREERTOS_config.h"

//setup//=========================================================================================

void setup() {
  Serial.begin(2000000);    //common init
  DDRB = B11111001;
...
void Task_current_input(void *pvParameters){
    ...
    while(1){
      ...
      if(current_read >= wheel_current_max){
        l_pwm_max /= 2;
      }else if(wheel_current_max <= 1900){
        l_pwm_max += 100;
      }
      //..............................................................
      ...
      if(current_read >= weapon_current_max){
        wp_pwm_max /= 2;
      }else if(weapon_current_max <= 1900){
        wp_pwm_max += 100;
      }
...
void Task_wppwm_output(void *pvParameters){
  (void) pvParameters;
  while(1){
    if(wp_pwm > wp_pwm_max){
      wp_pwm = wp_pwm_max;
    }
    Serial.print("wp_pwm_max: ");
    Serial.println(wp_pwm_max);
    Serial.print("wp_pwm: ");
